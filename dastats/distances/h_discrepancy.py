...
def to_device(iterator, device):
    ...
    for x in iterator:
        if type(x) == dict:
            yield {k : safe_to(v, device) for k,v in x.items()}
        else:
            arr = []
            for xi in x:
                if type(xi) == dict:
                    xi = {k : safe_to(v, device) for k,v in xi.items()}
                    arr.append(xi)
                else:
                    arr.append(safe_to(xi, device))
            yield tuple(arr)
...
class Discrepancy:
    ...
    def __init__(self, model_init, 
        ...
        device='cpu', verbose=False, very_verbose=False):
        """
        ...
        train_dataloader = lazy_kwarg_init(loader,
            batch_size=batch_size, 
            shuffle=True)
        test_dataloader = lazy_kwarg_init(loader,
            batch_size=batch_size, 
            shuffle=False, 
            drop_last=False)
        if very_verbose:
            self.train_dataloader = lambda x: tqdm(train_dataloader(x))
            self.test_dataloader = lambda x: tqdm(test_dataloader(x))
        else:
    ...
    def _asymmetric_compute(self, h, a, b):
        ...
        with torch.no_grad():
            ...
            for x, _, _, h_pred, z in iterator:
                yhat = self.commands.score(x, model).argmax(dim=1)
